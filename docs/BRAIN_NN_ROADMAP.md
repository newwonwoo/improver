...
## 8. 실측 강화 결과 (2026-06-12) — 측정 우선, 정직 보고

> 로드맵 §5의 예상치(앙상블 0.749 등)는 **outdated**. 실제 현재값을 재측정.

### 8.1 진단 (코드 매핑 + 실측)
- "신경망" 대부분은 **룰 기반 + 얕은 sklearn**. 유일한 진짜 NN(TorchBrain)은
  **torch 미설치로 실행 불가** + 퇴행(F1 0.468 < baseline 0.516).
- **최강 단일 신호 TextBrain(TF-IDF)이 하드코딩으로 꺼져 있었음**.
- 데이터: verdict 2,994(TP 512/FP 2,189/BORDER 293), 카테고리당 양성 42~186 →
  BERT·GNN 등 대형 NN은 **오버피팅 확정**(로드맵 §6 위험과 일치).

### 8.2 현 프로덕션 앙상블 실측 (slm_ensemble_harness, 고정임계·in-sample)
| 카테고리 | F1 |  | 카테고리 | F1 |
|---|---|---|---|---|
| 구조 | 0.367 | | 거버넌스 | 0.480 |
| 공정성 | **0.659** | | 효율성 | 0.589 |
| 적법성 | 0.512 | | **TOTAL** | **0.539** |

### 8.3 OOF 스태킹 강화 실험 (slm_stack_experiment, 5-fold CV, best-threshold)
| 카테고리 | struct | poly | text | **STACK** |
|---|---|---|---|---|
| 구조 | 0.577 | 0.390 | 0.472 | **0.593** |
| 공정성 | 0.496 | 0.463 | 0.575 | 0.569 |
| 적법성 | 0.437 | 0.342 | **0.583** | 0.552 |
| 거버넌스 | 0.448 | 0.489 | 0.529 | **0.542** |
| 효율성 | 0.558 | 0.472 | 0.596 | **0.622** |
| **가중평균** | 0.500 | 0.432 | 0.556 | **0.575** |

### 8.4 정직한 결론
1. **Poly interaction은 순효과 음(0.432)** — 로드맵 §2A 기대와 반대. 작은 데이터
   오버피팅. **추진 보류**(게이밍 아닌 음성 결과 보고).
2. **가장 큰 단일 레버 = 꺼진 텍스트 신호 활성화**(struct 0.500 → text 0.556).
3. **스태킹(struct+poly+text 메타 + isotonic 보정) = 0.575** — 구조특징 단일 대비
   **+0.075**, 최강 단일 대비 +0.019, 게다가 **보정된 확률**(신뢰도 약점 해소).
4. **하이브리드가 최선**: 공정성은 프로덕션 추론경로(0.659)가 스택(0.569)보다 강함
   → 공정성=추론 유지, 구조/적법성/거버넌스/효율성=스택(구조 0.367→0.593 대폭↑).
   이는 dual_core 의 LEAD_HEART(공정성→reasoning) 직관과 일치.

### 8.5 산출물 (opt-in, 프로덕션 기본값 무변 — text_brain 패턴)
- `engine/slm/stack_brain.py` — StackBrain(로드/추론/보정확률).
- `scripts/slm_stack_experiment.py` — 5-fold OOF 정직 측정.
- `scripts/train_stack_brain.py` → `outputs/stack_brain_models.pkl`.
- tests/test_stack_brain.py (6) — 보정확률·프로덕션 무변 검증.

### 8.6 다음 단계 (ROI 순)
1. **하이브리드 라우팅 통합**: 카테고리별 (공정성=추론 / 그외=스택) 라우터를
   opt-in 백엔드로 — 측정상 +0.04~0.05 기대(구조 대폭 개선).
2. verdict 증강(룰 마이닝 → 양성 표본↑) → 스택 base 재학습.
3. TorchBrain 은 torch 미설치·퇴행 상태 → **보류**(대형 NN은 데이터 확보 후).
