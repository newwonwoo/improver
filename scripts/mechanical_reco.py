...
_DEFECT_TRIGGERS: dict[str, list[re.Pattern]] = {
    ...
    # 추출 트리거 — 실제 F-01 룰의 _STRONG 과 정합되게 '…할 수 없다/하지 못한다' 포함
    # (추출은 이미 발화한 조문에서만 돌므로 광범위해도 안전; gold: 항공안전법 §10 '등록할 수 없다').
    "F-01": [re.compile(r"[^.。\n]{0,40}(제한할\s*수\s*있다|제한한다|거부할\s*수\s*있다|"
                        r"할\s*수\s*없다|하지\s*못한다|적용하지\s*아니한다|박탈|금지한다)"),
             re.compile(r"(배제|취소|정지)")],
